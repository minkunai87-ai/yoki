...
test('today essential keeps the canonical Stats store and never writes Firebase', async ({ page }) => {
  ...
  await page.goto('http://127.0.0.1:4173/?yokiTestMode=1', { waitUntil: 'domcontentloaded' });
  ...
  expect(statsBeforeFilter.labels).toContain('오늘 복습');
  ...
  const afterReviews = await page.evaluate(async () => {
    ...
    return {
      ...
      audit: window.__YOKI_FIREBASE_AUDIT__
    };
  });
  ...
  expect(await page.evaluate(() => window.__YOKI_FIREBASE_AUDIT__.writes)).toBe(0);
